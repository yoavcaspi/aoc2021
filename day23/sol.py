...
class State(NamedTuple):
        ...
        for key in ["A0", "A1", "B0", "B1", "C0", "C1", "D0", "D1"]:
            ...
            if val.state == "final":
                ...
                if flag and self.free_to_move(val.x, x):
                    ...
                    # print(f"{key} -> final")
                    res = State(new_amphi, new_board, new_energy).sol(min_res)
                    if res > 0:
    ...
    def a_free(self):
        flag = False
        if self.board[2,3] == "." and self.board[3,3] in ["A", "."]:
            flag = True
        if flag:
            if self.board[3,3] == ".":
                return flag, 3, 3
    ...
    def b_free(self):
        flag = False
        if self.board[2,5] == "." and self.board[3,5] in ["B", "."]:
            flag = True
        if flag:
            if self.board[3,5] == ".":
                return flag, 3, 5
    ...
    def c_free(self):
        flag = False
        if self.board[2, 7] == "." and self.board[3, 7] in ["C", "."]:
            flag = True
        if flag:
            if self.board[3, 7] == ".":
                return flag, 3,  7
    ...
    def d_free(self):
        flag = False
        if self.board[2, 9] == "." and self.board[3, 9] in ["D", "."]:
            flag = True
        if flag:
            if self.board[3, 9] == ".":
                return flag, 3,  9
...
def sol(data: str) -> int:
    ...
    for key, val in amphi.items():
        if val.y == 3 and final_pos[key[0]] == val.x:
            amphi[key] = AmphiState("final", val.x, val.y)
    for key, val in amphi.items():
        if val.y == 2 and final_pos[key[0]] == val.x and board[3, final_pos[key[0]]] == key[0]:
            amphi[key] = AmphiState("final", val.x, val.y)
    state = State(amphi, board, 0)
    return state.sol1(10000000)
