...
def sol(data: str) -> int:
    ...
    for i, line in enumerate(data.splitlines()):
        numbers.append((i, Pair.from_line(line)))
...
def example():
    # pair = Pair.from_line("[[[[[9,8],1],2],3],4]")
    # pair.reduce()
    # assert pair.left.left.left.left == 0
    # assert pair.left.left.left.right == 9
    # p1 = Pair.from_line("[[[[4,3],4],4],[7,[[8,4],9]]]")
    # p2 = Pair.from_line("[1,1]")
    # p3 = p1 + p2
    # p3.reduce()
    assert sol(INPUT_A) == 3993
